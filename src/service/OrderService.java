...
public class OrderService {
	...
	public void makeOrder() {
		while(true) {
			...
			Menu m = MenuService.getInstance().findBy(no + 1); // 숫자 번호는 1번부터
			// 수량 입력
...
//			private Customer lc = cu.loginCustomer;
	...
	// 결제 취소
	public void cancle() {
		int no = Util.nextInt("결제를 취소하실 메뉴 번호를 선택하여 주십시오. > ");
		
		int amount = Util.nextInt("결제를 취소하실 수량을 선택하여 주십시오. > ");
	...
	public static void main(String[] args) {// 구동 연습 메서드
		...
		int no = Util.nextInt(" 1. 주문하기, 2. 결제 취소하기 3. findBy점검 ");
		switch (no) {
